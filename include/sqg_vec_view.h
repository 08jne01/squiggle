...
namespace sqg
{
    ...
    template<concepts::read_vec3_type V>
    extend4_view<V> extend4( const V& vector )
    {
        return { vector };
    }
    ...
    struct vec_traits<extend4_view<V>>
    {
        using scalar_type = vec_traits<V>::scalar_type;
        using type = vec4<scalar_type>;
        using view = extend4_view<V>;
        static constexpr int n_dims = 4;

        static SQUIGGLE_INLINE constexpr scalar_type X(const view& v) { return sqg::X(v.vector); }
        static SQUIGGLE_INLINE constexpr scalar_type Y(const view& v) { return sqg::Y(v.vector); }
        static SQUIGGLE_INLINE constexpr scalar_type Z(const view& v) { return sqg::Z(v.vector); }
        static SQUIGGLE_INLINE constexpr scalar_type W(const view& v) { return scalar_type{1}; }
    };
}